--1. Consultas que dependem de Filtro de Data e Múltiplos JOINs como 5.2, 5.4, 5.5, 5.6, 5.7
--2. Consultas que Dependem de Agregação e Colunas de Ligação como 5.3, 5.8
--3. Consultas com Alto Custo Computacional 5.9

--CRIAÇÃO DE ÍNDICES

CREATE INDEX IF NOT EXISTS idx_order_purchase_timestamp ON olist_order (order_purchase_timestamp);

CREATE INDEX IF NOT EXISTS idx_order_status ON olist_order (order_status);

CREATE INDEX IF NOT EXISTS idx_order_customer_id ON olist_order (customer_id);

CREATE INDEX IF NOT EXISTS idx_order_cust_time ON olist_order (order_purchase_timestamp, customer_id);

CREATE INDEX IF NOT EXISTS idx_order_item_order_id ON olist_order_item (order_id);

CREATE INDEX IF NOT EXISTS idx_order_item_seller_id ON olist_order_item (seller_id);

CREATE INDEX IF NOT EXISTS idx_geolocation_zip_prefix ON olist_geolocation (geolocation_zip_code_prefix);

CREATE INDEX IF NOT EXISTS idx_payment_order_id ON olist_order_payment (order_id);

CREATE INDEX IF NOT EXISTS idx_review_order_id ON olist_order_review (order_id);
...
SELECT
    ...
    ROUND(SUM(t3.price + t3.freight_value)::NUMERIC, 2) AS valor_total_gasto
FROM
...
WITH cliente_localizacao AS (
    -- 1. SUBSTITUA A LATITUDE, LONGITUDE E RAIO ABAIXO
    SELECT
...
calculo_distancia AS (
    -- Aplica o Bounding Box como otimização
    SELECT
        vl.seller_id,
        (6371 * 2 * ASIN(
            SQRT(
    ...
    WHERE
        vl.lat_vendedor BETWEEN (cl.lat_cliente - 0.5) AND (cl.lat_cliente + 0.5) -- Otimização Bounding Box Lat
        AND vl.lng_vendedor BETWEEN (cl.lng_cliente - 0.5) AND (cl.lng_cliente + 0.5) -- Otimização Bounding Box Lng
)
SELECT
    cd.seller_id,
...
--Sim, a estrutura das consultas, o tipo de processamento de dados, e a manutenção do banco de dados.